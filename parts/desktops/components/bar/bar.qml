...
ShellRoot {
	...
	// src: https://github.com/melatonia/meloworld-dotfiles/blob/mangowm/quickshell/bar/WorkspaceBar.qml
	...
	function parseLine(line) {
		...
		try {
			...
			for (var i = 0; i < monitors.length; i++) {
				if (monitors[i]["monitor"] === root.screen.name) {
					monitorData = monitors[i]
					break
				}
			}
	...
	readonly property color hoveredColor: Qt.rgba(0, 0.5, 0.5, 0.98)
		...
		Repeater {
			...
			delegate: Rectangle {
				...
				Text {
					...
					text:             pill.tagNum
					color:            pill.isFocused ? Qt.rgba(1, 0, 0, 1) : Qt.rgba(1, 0, 0, 0.5)
					font.pixelSize:   16
					font.bold:        true
